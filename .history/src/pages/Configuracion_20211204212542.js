...
const Configuracion = () => {
  ...
  console.log(db.collection(user.email) ? "existe" : "no");
  const eliminarCuenta = () => {
    changeLoading(true);
    if (inputDeleteAcc.toLowerCase() === "eliminar mi cuenta") {
      if (db.collection(user.email)) {
        db.collection(user.email).delete();
      }
      user
        .delete()
        .then(() => {
        ...
        .catch((error) => {
          ...
          switch (error.code) {
            case "auth/requires-recent-login":
              msg =
                "Esta operación es sensible y requiere una autentificación reciente. Inicie sesión de nuevo e inténtelo otra vez.";
              break;
            default:
              msg =
                "Ocurrió un error al intentar borrar su cuenta, inténtelo más tarde.";
              break;
          }
          mostrarAlerta(true, "error", msg);
        });
      ...
      mostrarAlerta(true, "error", "Verifique el escrito de su confirmación.");
      changeLoading(false);
    }
  };
              ...
              <>
                ...
                <div className="warning">
                  ...
                  <Input
                    ...
                    onChange={handleChange}
                  />
                  <button className="btn-editar" onClick={eliminarCuenta}>
                    Hasta pronto... :(
                  </button>
                </div>
              </>
...
const ListOptions = styled.ul`
  ...
  .warning {
    ...
    padding: 10px 15px;
    animation: ${slideDown} 0.3s 0.3s ease forwards;
    user-select: none;
