...
const Configuracion = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (inputAEditar === "btnEditarEmail") {
      changeShowInput(inputAEditar);
    }
  };
  ...
  const updateEmail = () => {
    ...
    if (inputEmailAnterior === "" || inputEmailNuevo === "") {
    ...
    if (inputEmailAnterior === user.email) {
      ...
      if (!regexEmail.test(inputEmailAnterior)) {
        mostrarAlerta(
          true,
          "error",
      ...
      if (inputEmailNuevo !== "") {
        if (!regexEmail.test(inputEmailNuevo)) {
          mostrarAlerta(
            true,
            "error",
            "Por favor, ingrese un correo electrónico válido."
        ...
        user
          ...
          .catch((error) => {
            mostrarAlerta(
              true,
              "error",
              "Ocurrió un error al intentar cambiar su correo."
            );
          });
      }
    } else {
