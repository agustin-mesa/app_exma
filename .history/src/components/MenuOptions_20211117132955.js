...
const MenuOptions = () => {
  ...
  const handleMenuFocus = (e) => {
    setTimeout(() => changeShowMenu(!showMenu), 90);
  };
  const handleMenuOnBlur = (e) => {
    setTimeout(() => changeShowMenu(false), 90);
  };

  return (
    <>
      <Photo tabIndex="1" onBlur={handleMenuOnBlur} onFocus={handleMenuFocus}>
        {auth.currentUser ? (
